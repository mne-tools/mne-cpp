...
3. Create a new feature-related branch from `v2.0-dev`. Do not issue a Pull Request from your fork's `v2.0-dev` branch. Make your changes to that feature-branch commit.
...
7. Once all CI tests pass, go to your remote (forked GitHub) repository via the GitHub website and create a pull request:
   ...
   * Select the appropriate base and compare branch. For normal contributions, set the base branch to `v2.0-dev`. The compare branch is the branch where you implemented the new feature(s), bugfix(es), etc.
   * If GitHub shows "This branch cannot be rebased due to conflicts", rebase your feature branch onto `upstream/v2.0-dev` locally, resolve conflicts, and push your branch again (force push may be required).
