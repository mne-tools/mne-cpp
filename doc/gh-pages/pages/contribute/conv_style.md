...
The following steps will show you how to setup the MNE-CPP QtCreator wizard. This wizard is of great help to create new C++ classes which are conform with the MNE-CPP coding conventions. This helps streamlining the contribution process.
 ...
 2. Copy the `mnecpp` folder to your QtCreator the following location. Depending on your operating system, QtCreator will for wizards in different locations ([see Qt documentation](https://doc.qt.io/qtcreator/creator-project-wizards.html) for more information). As an example, on Windows, if you installed your Qt instance in `C:\Qt` you could copy the `mnecpp` folder to `C:\Qt\Tools\QtCreator\share\qtcreator\templates\wizards`. On Mac, you could copy to `/Users/<userName>/Qt Creator.app/Contents/Resources/templates/wizards`. And on Linux, `/home/<userName>/Qt/share\qtcreator\templates\wizards\`.
 3. Restart QtCreator.
