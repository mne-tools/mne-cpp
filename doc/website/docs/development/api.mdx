...
| [**Mna**](api-mna) | `MNALIB` | MNA graph engine: analysis pipeline specification (`.mna`/`.mnx`), operator schemas, batch and stream execution |
| [**Ml**](api-ml) | `MLLIB` | Machine learning: ONNX Runtime inference, linear models, feature pipelines, scalers, tensors |
| [**Sts**](api-sts) | `STSLIB` | Statistics: t-tests, F-tests, cluster permutation, covariance estimators (Ledoit-Wolf, shrinkage), source-level metrics |
