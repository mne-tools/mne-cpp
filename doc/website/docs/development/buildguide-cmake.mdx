...
- [Skigen](https://github.com/skigen-project/skigen) — header-only ML library (scikit-learn algorithms on Eigen), cloned from the `staging` branch
...
3. After installation, add the Qt `bin` folder to your `PATH`:
   The default Qt version is recorded as `QT_VERSION` in `src/external/external_deps.env`.
   - **Windows:** e.g. `C:\Qt\<version>\msvc2022_64\bin`
   - **Linux:** e.g. `$HOME/Qt/<version>/gcc_64/bin` — also add the `lib` folder to `LD_LIBRARY_PATH`
   - **macOS:** e.g. `$HOME/Qt/<version>/macos/bin` — also add the `lib` folder to `DYLD_LIBRARY_PATH`
...
2. When prompted, select the desktop kit for the Qt version in `src/external/external_deps.env`.
...
```bash
./scripts/deploy/deploy.bat dynamic pack    # dynamic linking (recommended for development)
./scripts/deploy/deploy.bat static pack     # static linking
```
