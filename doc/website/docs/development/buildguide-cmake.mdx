...
That keeps local development aligned with the dependency artifacts used in CI and release workflows. Qt currently downloads from the public `qt_binaries` prerelease tag. Eigen is set up to use the public `eigen_artifacts` prerelease tag once it exists; until then the init scripts transparently fall back to the bundled `src/external/eigen-5.0.1` tree.
...
If you ran `./init.sh` or `.\init.bat` first, `./scripts/build_project.bat` will also auto-detect the downloaded Qt bundle from `src/external/qt/`.
