...
3. After installation, add the Qt `bin` folder to your `PATH`:
   - **Windows:** e.g. `C:\Qt\6.11.2\msvc2022_64\bin`
   - **Linux:** e.g. `$HOME/Qt/6.11.2/gcc_64/bin` — also add the `lib` folder to `LD_LIBRARY_PATH`
   - **macOS:** e.g. `$HOME/Qt/6.11.2/macos/bin` — also add the `lib` folder to `DYLD_LIBRARY_PATH`
...
2. When prompted, select a kit with your Qt 6 installation — e.g. `Desktop Qt 6.11.2 MSVC2022 64bit`.
