...
Always work on a dedicated branch — never commit directly to `v2.0-dev`:

```bash
git fetch upstream
git checkout -b my-feature upstream/v2.0-dev
```
...
2. Set the **base repository** to `mne-tools/mne-cpp` and the **base branch** to `v2.0-dev`.
...
If GitHub shows **"This branch cannot be rebased due to conflicts"** for your PR, your feature branch and `v2.0-dev` both modified overlapping lines. Rebase locally, resolve conflicts, and push the updated branch:
...
Keep your fork's `v2.0-dev` branch up to date:
