...
# Bootstrap dependencies (Qt, Eigen) and configure
./init.sh

# Build with tests enabled
cmake -B build/developer-dynamic -S . -DBUILD_TESTS=ON
cmake --build build/developer-dynamic --parallel

# Run all tests
cd build/developer-dynamic
...
2. Set the **base repository** to `mne-tools/mne-cpp` and the **base branch** to **`staging`**.
...
- Once all CI checks pass and the reviewers approve, your code will be merged into `staging`.
