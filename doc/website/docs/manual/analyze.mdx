...
MNE Analyze is available as an experimental [WebAssembly](online) build. It runs entirely **locally** inside the browser — no installation is needed and no data are uploaded to any server. The browser merely acts as a cross-platform runtime environment, which is especially relevant when working with clinical MEG/EEG data containing protected health information.

**[Launch MNE Analyze in your browser](https://mne-cpp.github.io/wasm/mne_analyze.html)** · [Learn more about WebAssembly & data privacy](online)
