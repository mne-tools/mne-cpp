...
MNE Browse provides a lightweight interface for browsing raw MEG/EEG data in FIFF format. It does not depend on Qt3D or OpenGL, which makes it well suited for the WebAssembly platform. Supported features:
...
<div style={{display: 'flex', alignItems: 'center', gap: '0.75rem', marginBottom: '1rem'}}>
  <span>[Full documentation](anonymize) &nbsp;·&nbsp; **[Launch in browser](https://mne-cpp.github.io/wasm/mne_anonymize.html)**</span>
</div>
...
The WebAssembly build allows loading, anonymizing, and downloading a FIFF file entirely within the browser, with no data leaving the local machine. See the [full documentation](anonymize) for details on GUI and CLI modes.
...
- **Limited 3D visualization.** Qt3D and `QOpenGLWidget` support in WebAssembly is experimental. Some 3D views may not render as expected across all browsers.
