...
MNE Anonymize is a strong candidate for a future [WebAssembly](online) build. A browser-based version would allow clinicians and researchers to open a web page, load a FIFF file, anonymize it, and download the result — **all without any data ever leaving the local machine**. WebAssembly runs entirely inside the browser's sandbox; the browser merely acts as a runtime, comparable to running a desktop application. Other MNE-CPP applications — [MNE Browse](browse-raw) and [MNE Analyze](analyze) — can already be [run directly in the browser](online) today.
