...
MNE Inspect depends on the `mne_disp3D` library and is only built when this library is available. It also links against `mne_fs`, `mne_fiff`, `mne_fwd`, `mne_mne`, `mne_disp`, `mne_inv`, `mne_utils`, and `mne_conn`.
