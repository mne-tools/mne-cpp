---
title: The MEG/EEG Processing Workflow
sidebar_label: Workflow
sidebar_position: 1
---

# The MEG/EEG Processing Workflow

## Overview

This section describes the typical MEG/EEG processing workflow — from raw data through to source reconstruction — as originally outlined in the MNE software manual (Hämäläinen, 2009). It serves as both a conceptual overview and a practical step-by-step guide with concrete command examples. The workflow applies to both MNE-CPP command-line tools and the GUI applications.

The following diagram shows the complete workflow of the MNE software. The MEG/EEG data path and the MRI processing path converge at the forward solution computation.
...
Sometimes some MEG or EEG channels are not functioning properly for various reasons. These channels should be excluded from analysis by marking them as bad. Use **[mne_mark_bad_channels](tools-mark-bad-channels)** to store the bad channel list directly in the FIFF file:
...
SSP vectors can also be derived from a noise covariance matrix using **[mne_cov2proj](tools-cov2proj)**, which extracts the leading eigenvectors of the noise covariance as projection operators.
...
This creates the directories `mri/<name>-neuromag/slices` and `mri/<name>-neuromag/sets` and a corresponding `COR.fif` in the appropriate sets directory. If the input MRI data are in MGZ format, the output FIFF file will include the MRI pixel data and coordinate transformations between MRI (Surface RAS), MNI, and FreeSurfer Talairach coordinates.
...
- Optionally adding patch statistics with **[mne_add_patch_info](tools-add-patch-info)** for more accurate forward computation.
- For subcortical analysis, creating a volumetric source space with **[mne_volume_source_space](tools-volume-source-space)**.
- Verifying the source space with **[mne_list_source_space](tools-list-source-space)**.
- Converting annotation files to label files with **[mne_annot2labels](tools-annot2labels)** for ROI-based analysis.
...
Example using [mne_make_source_space](tools-make-source-space):

```bash
mne_make_source_space --subject duck_donald --spacing 5
```

The files are created in the `bem` directory:
...
:::tip
After the geometry is set up, verify that the source space points are located on the cortical surface by loading the `COR.fif` file and overlaying the corresponding `.pnt` or `.dip` files.
:::
...
Alternatively, **[mne_make_sphere_bem](tools-make-sphere-bem)** can create a spherical BEM model from concentric shells when MRI data is not available.

After creating the BEM surfaces, use **[mne_check_surface](tools-check-surface)** to validate the meshes for topological correctness (closed surfaces, correct normals, no intersections) before proceeding.
...
After the BEM is set up, use **[mne_prepare_bem_model](tools-prepare-bem-model)** to precompute the geometry matrices (the BEM solution) needed by the forward solver. It is also advisable to check that the BEM model meshes are correctly positioned.
...
When multiple forward solutions exist from different head positions or sessions, they can be combined with **[mne_average_forward_solutions](tools-average-forward-solutions)**. Use **[mne_sensitivity_map](tools-sensitivity-map)** to evaluate the spatial coverage of the sensor array.
...
- **[MNE Analyze](analyze)** — Interactive analysis tool for exploring data, producing quantitative results, screen snapshots, and movies.
- **[`mne_compute_mne`](tools-compute-mne)** / **[`mne_compute_raw_inverse`](tools-compute-raw-inverse)** — Command-line tools for computing source estimates (MNE, dSPM, sLORETA) from evoked or raw data.
- **[MNE Inspect](inspect)** — 3D visualization of source estimates on cortical surfaces, with atlas overlays, connectivity networks, and animated playback.
- **Label-based analysis** — Use `mne_compute_raw_inverse --labeldir` to extract time courses for cortical regions of interest (ROIs).
...
The following tools support group analysis workflows:

- **[mne_make_morph_maps](tools-make-morph-maps)** — Precompute surface morphing maps between subjects and the template (e.g., `fsaverage`).
- **[mne_morph_labels](tools-morph-labels)** — Morph cortical label (ROI) definitions across subjects.
- **[mne_smooth](tools-smooth)** — Apply spatial smoothing to source estimates for visualization or analysis.

### Data Import

MNE-CPP supports importing data from various acquisition systems. In addition to the native FIFF format and EDF conversion with **[mne_edf2fiff](tools-edf2fiff)**, the following converters are available:

- **[mne_brain_vision2fiff](tools-brain-vision2fiff)** — BrainVision EEG data (`.vhdr`)
- **[mne_ctf2fiff](tools-ctf2fiff)** — CTF MEG datasets (`.ds`). After conversion, use **[mne_compensate_data](tools-compensate-data)** to adjust the gradient compensation grade.
- **[mne_kit2fiff](tools-kit2fiff)** — KIT/Yokogawa MEG data (`.sqd`/`.con`)
- **[mne_raw2mat](tools-raw2mat)** — Export FIFF data to MATLAB format for interoperability.

Use **[mne_show_fiff](tools-show-fiff)** and **[mne_compare_fif_files](tools-compare-fif-files)** to inspect and validate FIFF files at any point in the pipeline.

### Simulation

**[mne_simu](tools-simu)** generates synthetic MEG/EEG data from a forward solution with controlled source location, frequency, and noise level. This is useful for validating inverse methods and testing processing pipelines end-to-end.
...
- [FIFF Format](fiff-format) — File format reference and conventions
- [Sample Dataset](sample-dataset) — Step-by-step walkthrough with the MNE sample data
- [Data Conversion](data-conversion) — Importing data from other MEG/EEG systems
- [Glossary](glossary) — MEG/EEG terminology and definitions
