...
The recursive feedback structure of CMNE is a defining feature that distinguishes it from both classical minimum-norm methods and conventional deep learning post-processing. Understanding it as a **Markov chain** provides key insights into the method's behaviour and stability.

#### Why CMNE forms a Markov chain

A **Markov chain** is a stochastic process in which the future state depends only on the current state (or, for higher-order chains, on a finite number of past states), not on the full history. In CMNE, the corrected source estimate $b_t$ depends on:

1. The current dSPM estimate $\hat{q}_t$ (derived from the physics-based inverse model), and
2. The **preceding $k$ corrected estimates** $b_{t-k}, \ldots, b_{t-1}$ (the look-back window).

This constitutes a **$k$-th order Markov chain** on the source state space $\mathbb{R}^Q$:

$$
b_t = g(b_{t-k}, \ldots, b_{t-1}, \hat{q}_t; \theta)
$$

where $g(\cdot)$ encapsulates the LSTM prediction, normalization, and element-wise multiplication. The state at time $t$ is fully determined by the most recent $k$ states and the current measurement — no memory beyond the look-back window influences the estimate.

#### Consequences of the Markov property

**Context accumulation.** Because each $b_t$ feeds into the window for computing $b_{t+1}$, the network progressively accumulates temporal context. Even though the window is finite ($k$ steps), information from earlier time points propagates forward through the chain: $b_{t-k}$ was itself computed from a window that included $b_{t-2k}$, and so forth. This creates an effective receptive field that grows linearly with the number of elapsed time steps, while keeping the computational cost per step constant.

**Causal processing.** The chain is strictly causal — $b_t$ depends only on past states, never on future ones. This is essential for real-time applications where future data is not yet available. In MNE Scan (real-time MEG), the CMNE correction can be applied sample-by-sample as new data arrives.

**Warm-up phase.** For the first $k$ time steps, the chain has not yet accumulated enough history to make meaningful corrections. During this phase, $b_t = \hat{q}_t$ (uncorrected dSPM). The estimates improve once the window is fully populated.

**Stability.** The normalization step $W_t^\text{CMNE}(i,i) = |p_t(i)| / \max_j |p_t(j)|$ bounds the correction weights to $[0, 1]$, preventing the recursive feedback from amplifying noise or diverging. This is a critical design choice: without it, the chain could exhibit positive feedback loops where artifacts in early estimates grow over time. The LSTM learns which sources to suppress (weight $\approx 0$) and which to preserve (weight $\approx 1$), effectively acting as a learned, time-varying spatial filter.

#### Connection to the paper

The Markov chain formulation is described in Dinh et al. (2021, Section 2.3). The paper demonstrates that the recursive structure is critical for CMNE's performance — a non-recursive variant (where the LSTM operates on uncorrected dSPM rather than on its own previous outputs) achieves significantly worse spatial resolution. The feedback loop allows the network to:

- **Track propagating activity**: as neural activation moves from one cortical area to another, the Markov chain naturally follows this trajectory because each prediction is conditioned on the recent corrected pattern.
- **Suppress spurious activations**: a source that was correctly suppressed in $b_{t-1}$ remains suppressed in the input to the LSTM at time $t$, reinforcing the correct spatial pattern.
- **Adapt to individual subjects**: fine-tuning the LSTM on a subject's own data allows the Markov chain to learn subject-specific temporal dynamics.
...
The **Mixed-Norm Estimate** (MxNE; Gramfort et al., 2012) promotes spatial sparsity while allowing temporal smoothness within each active source. It minimizes a cost function with an $\ell_{2,1}$-norm (group lasso) penalty:

$$
\min_{X} \frac{1}{2} \| M - GX \|_F^2 + \alpha \sum_{i=1}^{Q} \| X_i \|_2
$$

where $M$ is the $N \times T$ measurement matrix, $G$ is the $N \times Q$ gain matrix, $X$ is the $Q \times T$ source matrix, and $X_i$ denotes the $i$-th row of $X$ (the time course of source $i$). The parameter $\alpha > 0$ controls the trade-off between data fit and sparsity.

The $\ell_{2,1}$-norm $\sum_i \|X_i\|_2$ is the sum of $\ell_2$-norms of the rows: it penalizes the number of active sources (rows with non-zero $\ell_2$-norm) rather than individual time-point amplitudes. This encourages entire source time courses to be driven to zero, producing a solution where only a few sources are active across all time points.

#### IRLS Algorithm

MNE-CPP solves the MxNE problem using **Iteratively Reweighted Least Squares** (IRLS):

1. Initialize all source weights $w_i = 1$.
2. Construct the diagonal weight matrix $W = \operatorname{diag}(1/w_i^2)$.
3. Solve the weighted least-squares problem:
   $$
   X^{(k)} = (G^\top G + \alpha W)^{-1} G^\top M
   $$
4. Update the weights: $w_i = \max(\|X_i^{(k)}\|_2, \, \epsilon)$ where $\epsilon = 10^{-10}$.
5. Prune sources with $w_i < 10^{-8}$ from the active set.
6. Repeat from step 2 until convergence (maximum weight change $< \text{tolerance}$) or a maximum number of iterations is reached.
...
| `tolerance` | $10^{-6}$ | Convergence threshold on weight change |

#### References

- Gramfort, A.; Kowalski, M.; Hämäläinen, M.S. (2012). Mixed-norm estimates for the M/EEG inverse problem using accelerated gradient methods. *Physics in Medicine and Biology*, 57(7), 1937–1961. [DOI: 10.1088/0031-9155/57/7/1937](https://doi.org/10.1088/0031-9155/57/7/1937)
- Gramfort, A.; Strohmeier, D.; Haueisen, J.; Hämäläinen, M.S.; Kowalski, M. (2013). Time-frequency mixed-norm estimates: sparse M/EEG imaging with non-stationary source activations. *NeuroImage*, 70, 410–422. [DOI: 10.1016/j.neuroimage.2012.12.051](https://doi.org/10.1016/j.neuroimage.2012.12.051)
...
$$
M = G X + E, \qquad E \sim \mathcal{N}(0, C_n)
$$

where $C_n$ is the noise covariance matrix. Each source $i$ is assigned an independent Gaussian prior with unknown variance $\gamma_i$:

$$
X_i \sim \mathcal{N}(0, \gamma_i I_T)
$$

The model evidence (data covariance) is:

$$
C_M = G \, \Gamma \, G^\top + C_n, \qquad \Gamma = \operatorname{diag}(\gamma_1, \ldots, \gamma_Q)
$$
...
$$
\hat{X} = \Gamma \, G^\top \, C_M^{-1} \, M
$$

**M-step** — update source variances:

$$
\gamma_i^{\text{new}} = \frac{\| \hat{X}_i \|_2^2}{T}
$$

Sources with $\gamma_i < \gamma_{\text{threshold}}$ are pruned from the active set between iterations, reducing the dimensionality of $C_M$ and improving computational efficiency.

#### Convergence

The algorithm converges when the maximum relative change in $\gamma$ falls below a tolerance:

$$
\max_i \frac{|\gamma_i^{\text{new}} - \gamma_i^{\text{old}}|}{\max_j |\gamma_j^{\text{old}}|} < \text{tolerance}
$$
...
| `tolerance` | $10^{-6}$ | Convergence threshold on relative $\gamma$ change |
| `gammaThreshold` | $10^{-10}$ | Sources with $\gamma < \text{threshold}$ are pruned |

#### References

- Wipf, D. & Nagarajan, S.S. (2009). A unified Bayesian framework for MEG/EEG source imaging. *NeuroImage*, 44(3), 947–966. [DOI: 10.1016/j.neuroimage.2008.02.059](https://doi.org/10.1016/j.neuroimage.2008.02.059)
