...
Holds one `FIFFB_EVOKED` block: `times`, `data`, `first` / `last` sample indices, `baseline`, `kind`, `nave`, `comment` and the [`FiffInfo`](/docs/api/fiff/fiff-info) describing the channels. Use `FiffEvokedSet` for recordings that contain multiple averaging conditions.
...
- **p_FiffEvoked** : *const FiffEvoked &*
  [`Fiff`](/docs/api/fiff/fiff-class) evoked data which should be copied.

---

### ~FiffEvoked()

Destroys the [`FiffEvoked`](/docs/api/fiff/fiff-evoked).
...
Returns whether [`FiffEvoked`](/docs/api/fiff/fiff-evoked) is empty.
...
- *[FiffEvoked](/docs/api/fiff/fiff-evoked)* — the desired fiff evoked data.
...
- *[FiffEvoked](/docs/api/fiff/fiff-evoked) &* — the updated [`FiffEvoked`](/docs/api/fiff/fiff-evoked).
...
- **p_FiffEvoked** : *[FiffEvoked](/docs/api/fiff/fiff-evoked) &*
