...
Reads or writes one `QIODevice`. Builds the FIFF directory and [`FiffDirNode`](/docs/api/fiff/fiff-dir-node) block tree on `open` and exposes typed loaders / emitters for every structured record (id, channel info, coord trans, named matrix, dig point, projector, CTF comp, raw segment, evoked, covariance). The on-disk format is the Elekta/Neuromag FIFF specification — files emitted here round-trip through `mne.io.fiff` in MNE-Python without modification.

### Inheritance

```mermaid
classDiagram
    QDataStream <|-- FiffStream
```
...
- *[FiffId](/docs/api/fiff/fiff-id)* — The file identifier.
...
- *QList&lt; [FiffDirEntry](/docs/api/fiff/fiff-dir-entry)::SPtr &gt; &* — the directory.
...
- *const QList&lt; [FiffDirEntry](/docs/api/fiff/fiff-dir-entry)::SPtr &gt; &* — the directory.
...
- *const [FiffDirNode](/docs/api/fiff/fiff-dir-node)::SPtr &* — the compiled directory.
...
- **evoked_node** : *const QList&lt; [FiffDirNode](/docs/api/fiff/fiff-dir-node)::SPtr &gt; &*
  evoked tree nodes.
...
- **dentry** : *QList&lt; [FiffDirEntry](/docs/api/fiff/fiff-dir-entry)::SPtr &gt; &*
  The dir entries of which the tree should be constructed.

**Returns:**

- *[FiffDirNode](/docs/api/fiff/fiff-dir-node)::SPtr* — The created dir tree.
...
Reads the bad channel list from a node if it exists Note: In difference to mne-matlab this is not a static function. This is a method of the [`FiffDirNode`](/docs/api/fiff/fiff-dir-node) class, that's why a tree object doesn't need to be handed to the function.
...
Formerly mne_attach_env (SVN [MNE](/docs/api/mne/mne-class)).

The stream must already be opened for update (via open_update). Finds the first known top-level block ([MNE](/docs/api/mne/mne-class), MEAS, MRI, or BEM) and inserts a FIFFB_MNE_ENV block with working directory and command-line string.
...
- **p_digData** : *[FiffDigitizerData](/docs/api/fiff/fiff-digitizer-data) &*
...
- **p_InfoForward** : *[FiffInfoBase](/docs/api/fiff/fiff-info-base) &*
...
- **p_pTag** : *std::unique_ptr&lt; [FiffTag](/docs/api/fiff/fiff-tag) &gt; &*
  the read tag.
...
- **p_pTag** : *const std::unique_ptr&lt; [FiffTag](/docs/api/fiff/fiff-tag) &gt; &*
  Tag to write;.
...
- **chpos** : *const [FiffChPos](/docs/api/fiff/fiff-ch-pos) &*
...
- **p_FiffCov** : *const [FiffCov](/docs/api/fiff/fiff-cov) &*
...
- **comps** : *const QList&lt; [FiffCtfComp](/docs/api/fiff/fiff-ctf-comp) &gt; &*
...
- **dir** : *const QList&lt; [FiffDirEntry](/docs/api/fiff/fiff-dir-entry)::SPtr &gt; &*
...
- **id** : *const [FiffId](/docs/api/fiff/fiff-id) &*
  The id to write.
...
- **p_FiffInfoBase** : *const [FiffInfoBase](/docs/api/fiff/fiff-info-base) &*
...
- **mat** : *const [FiffNamedMatrix](/docs/api/fiff/fiff-named-matrix) &*
  The data matrix.
