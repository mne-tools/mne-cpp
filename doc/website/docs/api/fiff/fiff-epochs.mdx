...
Free / static helpers that turn a [FiffRawData](/docs/api/fiff/fiff-raw-data) plus an event list into fixed-length epochs.

Stateless — operates on the [FiffRawData](/docs/api/fiff/fiff-raw-data) and event arguments directly. Used by the source-reconstruction pipeline and by the offline averaging tooling when building epochs for an `FiffEvokedSet`.
...
All epochs must have the same dimensions. Returns a [FiffEvoked](/docs/api/fiff/fiff-evoked) with data, nave, times, and aspect_kind populated.
...
- *[FiffEvoked](/docs/api/fiff/fiff-evoked)* — FiffEvoked with averaged data and metadata.
