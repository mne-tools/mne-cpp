...
Maps one `FIFFB_PROJ_ITEM` block to a C++ value: description, kind, `nvec`, `active`, plus a [`FiffNamedMatrix`](/docs/api/fiff/fiff-named-matrix) whose row names are the channels the projector acts on. Multiple [`FiffProj`](/docs/api/fiff/fiff-proj) instances form the `info`['projs'] list and are combined into one projection operator by `make_projector` when raw / evoked data is loaded.
...
Destroys the [`FiffProj`](/docs/api/fiff/fiff-proj).
...
- **p_qListFiffProj** : *QList&lt; [FiffProj](/docs/api/fiff/fiff-proj) &gt; &*
...
- *QList&lt; [FiffProj](/docs/api/fiff/fiff-proj) &gt;* — List of [`FiffProj`](/docs/api/fiff/fiff-proj) items, or empty list on failure.
