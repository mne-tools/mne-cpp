...
On-disk layout is a 20-byte record (version + 2x4-byte machine ID + 8-byte [`FiffTime`](/docs/api/fiff/fiff-time)). The `machid` is filled from the host MAC address by ``FiffStream::write_id`` when generating new identifiers, matching the Neuromag acquisition stack so file lineage stays reconstructible.
...
- *[FiffId](/docs/api/fiff/fiff-id)* — A new unique [`FiffId`](/docs/api/fiff/fiff-id).
...
Returns a default [`FiffId`](/docs/api/fiff/fiff-id) object to be used as a mutable placeholder for a default instance of the class.

**Returns:**

- *[FiffId](/docs/api/fiff/fiff-id) &* — Reference to the default [`FiffId`](/docs/api/fiff/fiff-id) instance.
