...
Owns just the fields needed to interpret a data matrix: `chs`, `ch_names`, `nchan`, `sfreq`, `bads`, `dev_head_t`, `ctf_head_t`. Base class of [`FiffInfo`](/docs/api/fiff/fiff-info), used directly when the rest of the acquisition metadata is not available (e.g. realtime client streams, trimmed evoked files).

### Inheritance

```mermaid
classDiagram
    FiffInfoBase <|-- FiffInfo
    click FiffInfo "/docs/api/fiff/fiff-info"
```
...
- *[FiffInfoBase](/docs/api/fiff/fiff-info-base)* — Info modified according to sel.
