...
On-disk representation matches `mne.io.write.write_events` output and the `-eve.fif` files consumed by `mne.read_events`; the detection path mirrors `mne.find_events`. Used by [`FiffEpochs`](/docs/api/fiff/fiff-epochs) and the batch-averaging path in `FiffEvokedSet` to drive epoch extraction.
...
Constructs [`FiffEvents`](/docs/api/fiff/fiff-events) by reading from an I/O device.
...
- **p_Events** : *[FiffEvents](/docs/api/fiff/fiff-events) &*
  The loaded events.
