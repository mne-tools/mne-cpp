...
The [`AbstractFormatReader`](/docs/api/bids/abstract-format-reader) class provides a common interface for reading raw neurophysiology data from various file formats (EDF, BrainVision, etc.) used in BIDS datasets.
...
```mermaid
classDiagram
    AbstractFormatReader <|-- BrainVisionReader
    click BrainVisionReader "/docs/api/bids/brain-vision-reader"
    AbstractFormatReader <|-- EDFReader
    click EDFReader "/docs/api/bids/edf-reader"
```
