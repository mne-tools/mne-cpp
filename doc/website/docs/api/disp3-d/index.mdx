...
FSLIB::FsSurface surfLh("sample", 0, "inflated", subjectsDir);
FSLIB::FsSurface surfRh("sample", 1, "inflated", subjectsDir);
model->addSurface("sample", "lh", "inflated", surfLh);
model->addSurface("sample", "rh", "inflated", surfRh);

// Load annotations
FSLIB::FsAnnotation annotLh("sample", 0, "aparc", subjectsDir);
...
| Class | MNE-Python Equivalent |
|---|---|
| `BrainView` | `mne.viz.Brain` |
| `MultiViewLayout` | — (MNE-Python is single-view) |

### Render Engine

| Class | MNE-Python Equivalent |
|---|---|
| `BrainRenderer` | `Brain._renderer` |
| `CameraController` | `Brain._view.camera` |
| `RayPicker` | `Brain._on_pick()` |

### Renderable Objects

| Class | MNE-Python Equivalent |
|---|---|
| `BrainSurface` | `Brain._surface` |
| `DipoleObject` | `mne.viz.plot_dipole_locations()` |
| `NetworkObject` | — |
| `SourceEstimateOverlay` | `Brain.set_data_time_index()` |
| `MeshFactory` | — |

### Data Model (Tree Items)

| Class | MNE-Python Equivalent |
|---|---|
| `BrainTreeModel` | — |
| `AbstractTreeItem` | — |
| `SurfaceTreeItem` | FreeSurfer surface via `Brain.add_surface()` |
| `BemTreeItem` | BEM forward reference |
| `SensorTreeItem` | `Brain.get_sensors()` |
| `DipoleTreeItem` | — |
| `SourceSpaceTreeItem` | `Brain._source_space` |
| `NetworkTreeItem` | — |
| `DigitizerTreeItem` / `DigitizerSetTreeItem` | `mne.Digitization` |

### Data Loaders & Managers

| Class | MNE-Python Equivalent |
|---|---|
| `DataLoader` | `mne.io.read_raw_fif()`, `mne.read_source_spaces()` |
| `SourceEstimateManager` | — |
| `SensorFieldMapper` | `mne.viz.plot_evoked_field()` |
| `RtSensorStreamManager` | — (no real-time in Python) |

### Background Workers (Real-Time)

| Class | Description |
|---|---|
| `RtSensorDataWorker` | Apply interpolation matrix to incoming sensor samples |
| `RtSensorInterpolationMatWorker` | Build sparse sensor → surface mapping matrix |
| `RtSourceDataWorker` | Apply STC interpolation to incoming source estimates |
| `RtSourceInterpolationMatWorker` | Build sparse source → vertex mapping matrix |
| `StcLoadingWorker` | Asynchronous STC file I/O |

### Helpers

| Class | Description |
|---|---|
| `GeometryInfo` | Sensor-to-mesh mapping, surface-constrained distance computation |
| `Interpolation` | Distance-based weight matrices and signal interpolation |

## Doxygen Reference

For method signatures, inheritance diagrams, and source-level documentation see the auto-generated [**DISP3DLIB namespace**](https://mne-cpp.github.io/doxygen-api/namespaceDISP3DLIB.html) in the Doxygen API reference.

## See Also

- [Library API Overview](/docs/api) — All MNE-CPP libraries
- [Connectivity Library](/docs/api/connectivity/) — Connectivity metrics that produce `Network` objects for 3D visualisation
- [Inverse Library](/docs/api/inv/) — Source estimation that produces `SourceEstimate` for STC overlay
- [MNE-Python Brain API](https://mne.tools/stable/generated/mne.viz.Brain.html)
