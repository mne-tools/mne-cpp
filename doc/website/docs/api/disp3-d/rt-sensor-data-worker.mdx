...
[`RtSensorDataWorker`](/docs/api/disp3-d/rt-sensor-data-worker) processes incoming sensor measurement data in a background thread.
...
This worker is driven by a timer in the `RtSensorDataController` and follows the same architecture as [`RtSourceDataWorker`](/docs/api/disp3-d/rt-source-data-worker), adapted for dense sensor-to-surface mapping instead of sparse source interpolation.
