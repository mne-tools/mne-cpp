...
[`RtSourceInterpolationMatWorker`](/docs/api/disp3-d/rt-source-interpolation-mat-worker) computes sparse source-to-vertex interpolation matrices in a background thread.
...
[FsSurface](/docs/api/fs/fs-surface) vertex positions and neighbor topology
...
```mermaid
classDiagram
    QObject <|-- RtSourceInterpolationMatWorker
```
