...
- *QVector&lt; [MNEEpochData](/docs/api/mne/mne-epoch-data) &gt;* — Vector of MNEEpochData (one per valid event). Rejected epochs are included with MNEEpochData::bReject == true so callers can choose whether to exclude them.
...
- *QVector&lt; [MNEEpochData](/docs/api/mne/mne-epoch-data) &gt;* — Subset of epochs with bReject == false.
