---
id: connectivity-class
title: "CONNECTIVITYLIB::Connectivity"
...
Dispatcher that runs one or more functional-connectivity estimators over a batch of pre-processed trials and returns one [`Network`](/docs/api/connectivity/network) per estimator.

The selected methods are read from [ConnectivitySettings](/docs/api/connectivity/connectivity-settings)::getConnectivityMethods and matched by string against the supported metric set ("COR", "XCOR", "COH", "IMAGCOH", "PLI", "USPLI", "WPLI", "DSWPLI", "PLV", "GC", "DTF", "PDC"). All spectral metrics share the same DPSS tapered FFT and CSD accumulator inside [`ConnectivitySettings`](/docs/api/connectivity/connectivity-settings), so running e.g. PLI + wPLI + dwPLI in one call costs only one FFT pass.

Runs the selected functional-connectivity metrics over a [`ConnectivitySettings`](/docs/api/connectivity/connectivity-settings) batch.
...
Constructs a [`Connectivity`](/docs/api/connectivity/connectivity-class) object.
...
- *QList&lt; [Network](/docs/api/connectivity/network) &gt;* — Returns the list with calculated networks for each provided method.

---

## Example

Source: [`src/examples/ex_connectivity/main.cpp`](https://github.com/mne-tools/mne-cpp/tree/staging/src/examples/ex_connectivity/main.cpp)
