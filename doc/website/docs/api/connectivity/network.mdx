...
[Graph](/docs/api/disp/graph) container for one functional-connectivity result.

Holds the [`NetworkNode`](/docs/api/connectivity/network-node) list (channel / source positions) and the [`NetworkEdge`](/docs/api/connectivity/network-edge) list (per-frequency weight matrices + scalar band- averaged weights). Provides full and thresholded views of both edges and the corresponding adjacency matrix, the metric name ("COH", "PLI", "GC", ...) and threshold currently in effect, and the `VisualizationInfo` block read by the disp3D rendering layer.

[Graph](/docs/api/disp/graph) container for one connectivity metric; nodes + weighted edges + threshold/visualisation state.
...
Constructs a [`Network`](/docs/api/connectivity/network) object.
...
- *const QList&lt; QSharedPointer&lt;[NetworkEdge](/docs/api/connectivity/network-edge)&gt; &gt; &* — Returns the network edges.
...
- *const QList&lt; QSharedPointer&lt;[NetworkNode](/docs/api/connectivity/network-node)&gt; &gt; &* — Returns the network nodes.
...
- *QSharedPointer&lt;[NetworkEdge](/docs/api/connectivity/network-edge)&gt;* — Returns the network edge.
...
- *QSharedPointer&lt;[NetworkNode](/docs/api/connectivity/network-node)&gt;* — Returns the network node.
...
- **newEdge** : *QSharedPointer&lt;[NetworkEdge](/docs/api/connectivity/network-edge)&gt;*
  The new edge item.
...
- **newNode** : *QSharedPointer&lt;[NetworkNode](/docs/api/connectivity/network-node)&gt;*
...
Returns whether the [`Network`](/docs/api/connectivity/network) is empty by checking the number of nodes and edges.

**Returns:**

- *bool* — The flag identifying whether the [`Network`](/docs/api/connectivity/network) is empty.
