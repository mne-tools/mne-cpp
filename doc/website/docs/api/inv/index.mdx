...
**Namespace:** `INVERSELIB` &nbsp;·&nbsp; **Source:** `src/libraries/inv`
...
Reference: Dinh et al. (2021). *Contextual Minimum-Norm Estimates (CMNE): A Deep Learning Method for Source Estimation in Neuronal Networks.* Frontiers in Neuroscience, 15, 552666. [DOI: 10.3389/fnins.2021.552666](https://doi.org/10.3389/fnins.2021.552666)
...
**MxNE** minimizes a group-lasso ($\ell_{2,1}$-norm) cost function using iteratively reweighted least squares with active-set pruning. **Gamma-MAP** uses an EM algorithm to estimate per-source variance hyperparameters, pruning sources below a configurable threshold.
...
For method signatures, inheritance diagrams, and source-level documentation see the auto-generated [**INVLIB namespace**](https://mne-cpp.github.io/doxygen-api/namespaceINVLIB.html) in the Doxygen API reference.

## See Also

- [DSP Library](/docs/api/dsp/) — Signal processing algorithms
- [Connectivity Library](/docs/api/connectivity/) — Functional connectivity metrics
- [MNE Dipole Fit](/docs/manual/dipole-fit) — Application GUI for dipole fitting
- [mne_inverse_operator](/docs/manual/tools-inverse-operator) — CLI tool for inverse operator computation
