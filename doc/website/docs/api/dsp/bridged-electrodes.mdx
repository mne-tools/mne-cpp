...
The implementation operates on the raw broadband signal; no rereferencing or filtering is required up-front. Output is a list of ``(i, j) channel- index pairs that the caller can mark bad, interpolate, or pass into the [SSS](/docs/api/dsp/sss) bad-channel set.
