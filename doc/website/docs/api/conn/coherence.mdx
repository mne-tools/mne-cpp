...
**Namespace:** `CONNECTIVITYLIB` &nbsp;·&nbsp; **Library:** Connectivity Library &nbsp;·&nbsp; **Header:** `conn/metrics/coherence.h`

:::info[Python equivalent]
`mne_connectivity.spectral_connectivity_epochs (method='coh')` in MNE-Python.
:::

`#include <conn/coherence.h>`

```cpp
class CONNLIB::Coherence
```
