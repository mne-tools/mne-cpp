---
id: multi-view
title: "DISPLIB::MultiView"
sidebar_label: MultiView
sidebar_position: 43
---

# MultiView

**Namespace:** `DISPLIB` &nbsp;·&nbsp; **Library:** Display Library

`#include <disp/multiview.h>`

```cpp
class DISPLIB::MultiView
```

QMainWindow container hosting an arbitrary number of [`MultiViewWindow`](/docs/api/disp/multi-view-window) dock widgets.

Inherits `QMainWindow` purely for the dock infrastructure; named views are added with `addWidgetTop` / `addWidgetBottom` and geometry is persisted via `QSettings`.

### Inheritance

```mermaid
classDiagram
    QMainWindow <|-- MultiView
```

---

## Public Methods

### MultiView(sSettingsPath, parent, flags)

Constructs an [`MultiView`](/docs/api/disp/multi-view).

---

### ~MultiView()

Destructs an [`MultiView`](/docs/api/disp/multi-view).

---

### addWidgetTop(pWidget, sName)

Adds a QWidget to the top docking area.

**Parameters:**

- **pWidget** : *QWidget **
  The widget to be added.

- **sName** : *const QString &*
  The window title shown in the QSplitter.

**Returns:**

- *[MultiViewWindow](/docs/api/disp/multi-view-window) ** — Returns a pointer to the added widget in form of a [`MultiViewWindow`](/docs/api/disp/multi-view-window).

---

### addWidgetBottom(pWidget, sName)

Adds a QWidget to the bottom docking area.

Please note that all bottom dock widgets are tabbified by default.

**Parameters:**

- **pWidget** : *QWidget **
  The widget to be added.

- **sName** : *const QString &*
  The window title shown in the QSplitter.

**Returns:**

- *[MultiViewWindow](/docs/api/disp/multi-view-window) ** — Returns a pointer to the added widget in form of a [`MultiViewWindow`](/docs/api/disp/multi-view-window).

---

### saveSettings()

Saves geometry and state of GUI dock widgets that have given a name with setObjectName().

---

### loadSettings()

Restores geometry and state as saved by `saveSettings()`.

---

## Authors of this file

- Lorenz Esch &lt;lorenz.esch@tu-ilmenau.de&gt;
- Gabriel Motta &lt;gabrielbenmotta@gmail.com&gt;
- Juan GPC &lt;jgarciaprieto@mgh.harvard.edu&gt;
- Christoph Dinh &lt;christoph.dinh@mne-cpp.org&gt;
