...
Drives a [`SelectionScene`](/docs/api/disp/selection-scene) of layout dots; lasso / click selection updates the named groups stored in [MNE](/docs/api/mne/mne-class) ``.sel files and emits the resulting visible-channel list to the raw and butterfly viewers.
...
Constructs a [`ChannelSelectionView`](/docs/api/disp/channel-selection-view) which is a child of parent.

**Parameters:**

- **parent** : *QWidget **
  pointer to parent widget; If parent is 0, the new [`ChannelSelectionView`](/docs/api/disp/channel-selection-view) becomes a window. If parent is another widget, [`ChannelSelectionView`](/docs/api/disp/channel-selection-view) becomes a child window inside parent. [`ChannelSelectionView`](/docs/api/disp/channel-selection-view) is deleted when its parent is deleted.
...
Destroys the [`ChannelSelectionView`](/docs/api/disp/channel-selection-view).

All [`ChannelSelectionView`](/docs/api/disp/channel-selection-view)'s children are deleted first. The application exits if [`ChannelSelectionView`](/docs/api/disp/channel-selection-view) is the main widget.
