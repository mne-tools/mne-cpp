...
```mermaid
classDiagram
    QAbstractTableModel <|-- RtFiffRawViewModel
```
...
Destructs [`RtFiffRawViewModel`](/docs/api/disp/rt-fiff-raw-view-model) and stops the use of Shared Memory within the Event Manager.
...
- **filterData** : *QList&lt; [FilterKernel](/docs/api/dsp/filter-kernel) &gt;*
...
range value in [FiffChInfo](/docs/api/fiff/fiff-ch-info) does not seem to contain a reasonable value
