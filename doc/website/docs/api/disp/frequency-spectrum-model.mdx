...
QAbstractTableModel storing per-channel FFT magnitudes for [`SpectrumView`](/docs/api/disp/spectrum-view).
...
```mermaid
classDiagram
    QAbstractTableModel <|-- FrequencySpectrumModel
```
...
- *QSharedPointer&lt;[FiffInfo](/docs/api/fiff/fiff-info)&gt;* — the fiff info.
