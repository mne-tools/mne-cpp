...
Propagates baseline-correction, SSP-projection and bad-channel updates so connected views ([`ButterflyView`](/docs/api/disp/butterfly-view), [`AverageLayoutView`](/docs/api/disp/average-layout-view)) stay in sync with the latest pre-processing settings.

### Inheritance

```mermaid
classDiagram
    QAbstractTableModel <|-- EvokedSetModel
```
