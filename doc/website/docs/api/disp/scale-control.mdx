...
Supports linear and log10 mapping, configurable min / max and an optional inverted direction; building block of [`ScalingView`](/docs/api/disp/scaling-view) and several other settings panels.

### Inheritance

```mermaid
classDiagram
    QWidget <|-- ScaleControl
```
...
Constructs a [`ScaleControl`](/docs/api/disp/scale-control) object who's Qlabel element will be set.
...
Constructs a [`ScaleControl`](/docs/api/disp/scale-control) object who's Qlabel element will be set, and sets the parent of the underlying QWidget.
...
getUI Return a pointer to the GUI of the [`ScaleControl`](/docs/api/disp/scale-control).
