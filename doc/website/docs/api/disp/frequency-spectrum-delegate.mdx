...
QAbstractItemDelegate painting one [`SpectrumView`](/docs/api/disp/spectrum-view) row as a horizontal frequency band.
...
```mermaid
classDiagram
    QAbstractItemDelegate <|-- FrequencySpectrumDelegate
```
