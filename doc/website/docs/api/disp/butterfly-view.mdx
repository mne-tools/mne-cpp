...
Renders all visible traces in a single shader pass for smooth zoom / pan; visibility per modality and per condition is driven by [`ModalitySelectionView`](/docs/api/disp/modality-selection-view) and [`AverageSelectionView`](/docs/api/disp/average-selection-view). Used as the main evoked viewer in MNE-Scan.

### Inheritance

```mermaid
classDiagram
    QRhiWidget <|-- ButterflyView
```
...
Destroys the [`ButterflyView`](/docs/api/disp/butterfly-view).
...
- **model** : *QSharedPointer&lt;[EvokedSetModel](/docs/api/disp/evoked-set-model)&gt;*
