...
QAbstractItemDelegate painting one FIFF channel's signal trace inside [`RtFiffRawView`](/docs/api/disp/rt-fiff-raw-view).

Transforms each visible row of the [`RtFiffRawViewModel`](/docs/api/disp/rt-fiff-raw-view-model) into a polyline drawn inside the cell rectangle, applying the active y-scaling, SSP / compensation operators, trigger overlays and bad-channel greying.

### Inheritance

```mermaid
classDiagram
    QAbstractItemDelegate <|-- RtFiffRawViewDelegate
```
...
- **parent** : *[RtFiffRawView](/docs/api/disp/rt-fiff-raw-view) **
