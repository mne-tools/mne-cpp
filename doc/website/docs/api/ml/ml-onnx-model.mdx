...
[`MlModel`](/docs/api/ml/ml-model) backend that runs ``.onnx graphs through ONNX Runtime with a cached CPU session.
...
classDiagram
    MlModel <|-- MlOnnxModel
    click MlModel "/docs/api/ml/ml-model"
```
...
- **input** : *const [MlTensor](/docs/api/ml/ml-tensor) &*
  The input data.

**Returns:**

- *[MlTensor](/docs/api/ml/ml-tensor)* — The prediction result.
