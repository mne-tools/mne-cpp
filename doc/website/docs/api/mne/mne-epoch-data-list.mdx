...
Multi-category offline averaging. Reads epochs from raw data for multiple event types and returns an `FiffEvokedSet` with one [FiffEvoked](/docs/api/fiff/fiff-evoked) per category. Ported from average.c (MNE-C).
