...
[`MNEProjOp`](/docs/api/mne/mne-proj-op) aggregates zero or more [`MNEProjItem`](/docs/api/mne/mne-proj-item) entries, each holding a named matrix of projection vectors. When the operator is compiled (make_projector), the individual items are orthogonalised into a single dense projector stored in `proj_data`.
...
- **from** : *[MNEProjOp](/docs/api/mne/mne-proj-op) **
  Source operator whose items are copied.

**Returns:**

- *[MNEProjOp](/docs/api/mne/mne-proj-op) ** — Pointer to this operator.
...
- **vecs** : *const [MNENamedMatrix](/docs/api/mne/mne-named-matrix) **
...
- *std::unique_ptr&lt; [MNEProjOp](/docs/api/mne/mne-proj-op) &gt;* — A newly allocated copy. Caller takes ownership.
...
- **c** : *[MNECovMatrix](/docs/api/mne/mne-cov-matrix) **
...
- *std::unique_ptr&lt; [MNEProjOp](/docs/api/mne/mne-proj-op) &gt;* — A new projection operator, or NULL if no EEG channels are found. Caller takes ownership.
...
- **start** : *const [FIFFLIB::FiffDirNode](/docs/api/fiff/fiff-dir-node)::SPtr &*
  The tree node to search for projection blocks.

**Returns:**

- *std::unique_ptr&lt; [MNEProjOp](/docs/api/mne/mne-proj-op) &gt;* — A populated projection operator (possibly with zero items), or NULL on error. Caller takes ownership.
...
- *std::unique_ptr&lt; [MNEProjOp](/docs/api/mne/mne-proj-op) &gt;* — The loaded projection operator, or NULL on error. Caller takes ownership.
...
- **result** : *std::unique_ptr&lt; [MNEProjOp](/docs/api/mne/mne-proj-op) &gt; &*
