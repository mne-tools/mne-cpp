...
Implements the [`MNE`](/docs/api/mne/mne-class) Raw Data (Replaces *mneRawData,mneRawDataRec; struct of MNE-C `mne_types.h`).
...
Constructs the [`MNE`](/docs/api/mne/mne-class) Raw Data Refactored: new_raw_data (mne_raw_data.c).

---

### ~MNERawData()

Destroys the [`MNE`](/docs/api/mne/mne-class) Raw Data Refactored: mne_raw_free_data (mne_raw_data.c).
...
- **buf** : *[MNERawBufDef](/docs/api/mne/mne-raw-buf-def) **
...
Reads channel info, compensation data, [SSS](/docs/api/dsp/sss) data, sets up data/filter buffers and bad channels, and returns a fully initialized raw data object.
...
- **filter** : *const [MNEFilterDef](/docs/api/mne/mne-filter-def) &*
...
- *[MNERawData](/docs/api/mne/mne-raw-data) ** — A new raw data object, or NULL on failure. Caller takes ownership.
