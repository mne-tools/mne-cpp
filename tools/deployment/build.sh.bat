...
if [ "$(uname)" == "Darwin" ]; then
    ...
    if [[ ${PACK_OPTION} == TRUE ]]; then
        printf 'Pero que paisaaa!\n'
    fi

    if [[ ${LINK_OPTION} == dynamic ]]; then

        #Clone repository
        # cloneRepo --repo https://github.com/mne-tools/mne-cpp.git --cdIntoFolder mne-cpp --clone-submodules
        cloneRepo --cdIntoFolder mne-cpp --clone-submodules
