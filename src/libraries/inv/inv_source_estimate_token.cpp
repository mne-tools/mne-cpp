 ...
 * layers — emitting structural tokens around each section so the parser
 * can re-section the stream without look-ahead. Continuous values
 * (amplitudes, positions, times, correlations) are paired with their
 * @ref InvTokenId in the @c value field of @ref InvToken; structural
 * tokens leave it at zero. The inverse path is a single-pass state
 * machine that rebuilds each layer as its @c *Begin / @c *End brackets
 * are consumed.
 */
