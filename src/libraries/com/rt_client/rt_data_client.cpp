 ...
 * server-assigned routing handle, @c readInfo() / @c readMetadata()
 * stream the measurement header (and optional digitiser block) through
 * @ref FIFFLIB::FiffStream, and @c readRawBuffer() blocks until the
 * next sample frame arrives, reshaping the payload into an
 * @c Eigen::MatrixXf and reporting the tag kind so callers can
 * distinguish data buffers from block-control markers.
 */
