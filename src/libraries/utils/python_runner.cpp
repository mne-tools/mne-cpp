 ...
 * synchronously to the caller's line callback so progress
 * UIs update in real time. The runner deliberately keeps no
 * static state — several instances can run concurrently in
 * different threads (each owning its own @c QProcess) without
