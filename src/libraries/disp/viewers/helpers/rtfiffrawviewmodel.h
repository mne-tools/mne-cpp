 ...
 * @since 2022
 * @date  March 2026
 * @brief Circular-buffer QAbstractTableModel feeding the real-time FIFF raw browser.
 *
 * RtFiffRawViewModel keeps the last few seconds of streaming FIFF
 * data in a per-channel ring buffer and exposes it through Qt's
 * model API as one row per channel; the delegate uses the per-row
 * data vector and the current scaling / projection / compensation
 * operators to paint the trace. It also forwards bad-channel flips,
 * trigger detections and annotation edits back to the underlying
 * @c FiffInfo / event list.
 */
 ...
 * @brief Circular-buffer QAbstractTableModel feeding the real-time FIFF raw browser.
 *
 * Keeps the last few seconds of streaming FIFF data in a
 * per-channel ring buffer and forwards bad-channel flips, trigger
 * detections and annotation edits back to the underlying
 * @c FiffInfo / event list.
 */
