 ...
 *
 * Implements FIFF read of the @c FIFFB_MNE_INVERSE_SOLUTION block, the
 * @c prepare_inverse_operator step (whitener + SVD scaling for a given
 * SNR / depth weighting), conversion between fixed and free orientations
 * and the cluster-then-rebuild path used to evaluate clustered inverse
 * solutions.
 */
                                                             ...
                                                             bool limit_depth_chs)
{
    ...
    //
    qInfo("\tCreating the source covariance matrix\n");
    ...
    //
    qInfo("Computing SVD of whitened and weighted lead field matrix.\n");
...
MNEInverseOperator MNEInverseOperator::prepare_inverse_operator(qint32 nave ,float lambda2, bool dSPM, bool sLORETA) const
{
    ...
    qInfo("Preparing the inverse operator for use...\n");
    ...
    qInfo("\tCreated the regularized inverter\n");
    //
...
bool MNEInverseOperator::read_inverse_operator(QIODevice& p_IODevice, MNEInverseOperator& inv)
{
    ...
    qInfo("\tReading inverse operator info...");
    //
    ...
    if (inv.coord_frame != FIFFV_COORD_MRI && inv.coord_frame != FIFFV_COORD_HEAD)
        qCritical("Only inverse solutions computed in MRI or head coordinates are acceptable");
    //
...
void MNEInverseOperator::writeToStream(FiffStream* p_pStream)
{
    ...
    qInfo("\tWriting inverse operator info...\n");
    ...
    //
    qInfo("\tWriting noise covariance matrix.");
    p_pStream->write_cov(*this->noise_cov.data());

    qInfo("\tWriting source covariance matrix.\n");
